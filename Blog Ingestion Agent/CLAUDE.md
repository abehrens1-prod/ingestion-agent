...
**Image uploads via `upload_assets.py`.** `api.assets_url` in `config.yaml` points at the dedicated blog-post assets endpoint Dale (ITS) provided 2026-08-26 (`.../uploadToBlogAssetsFolder`) — POST, multipart with a single `body` field holding the file. Extracts embedded docx images and uploads them, patching `assetUid` into `normalized_blog.json` automatically. **Response shape confirmed** against a real upload (IDC Spotlight post, `assetUid: blt5aa365fd4729e2de`) — `resp_json["asset"]["uid"]` (or top-level `uid`) is correct.

This step now runs automatically, not manually: `run_pipeline.py` calls it as Step 1b right after parsing (gated on `api.assets_url` being set), and the `/blog-ingestion` skill runs it as Phase 3d, before mapping. Only fall back to running `python3 scripts/upload_assets.py output/<slug>/ input/blog.docx` by hand if that automatic step reports a failure for a specific image — a bad response shape, network error, or an image `python-docx` couldn't extract. If asset upload doesn't populate `assetUid` for an image, that block keeps its `sourcePath`/`url` placeholder and validation will WARN on it.
