...
Usage (standalone):
    ...
    python scripts/upload_to_contentstack.py output/2026-08-25_semantic-layer/contentstack_entry.json --publish
...
def upload_entry(
    ...
    content_type = config.get("contentstack", {}).get("content_type_uid", "asset_page")
    ...
    # create/update. Unverified against a real non-"en" upload — see config.yaml's
    # locale comment.
    ...
    if entry_uid:
        url = f"{base_url}/content_types/{content_type}/entries/{entry_uid}"
        ...
        # Strip any existing vN | prefix or trailing (...) version noise to get the base title
        raw_title = entry.get("title", "")
        base_title = re.sub(r'^v\d+\s*\|\s*', '', raw_title)   # strip leading "v3 | "
        base_title = re.sub(r'\s*\([^)]*\)\s*$', '', base_title)  # strip trailing "(v2 - fixed)" etc.
        base_title = base_title.strip()
        versioned_title = None
        r = None
        for version in range(1, 20):
            ...
            if r.status_code in (200, 201):
                versioned_title = candidate
                break
            # Only retry on title-uniqueness errors
            try:
        ...
        if versioned_title is None:
            ...
            raise RuntimeError(f"Entry creation failed with HTTP {r.status_code}")
        # Write the versioned title back to the entry file so the file stays in sync
