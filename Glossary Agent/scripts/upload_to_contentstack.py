"""Upload a Glossary Contentstack entry through the shared proxy client."""
...
def upload_entry(
    ...
    """Upload a Glossary entry while preserving the pipeline's public interface."""
    if config is None:
        config = load_config(config_path, project_root=PROJECT_ROOT)
    content_type = config.get("contentstack", {}).get(
